...
from slack_lists_server import (  # noqa: E402
    SlackListsClient,
    SlackListsError,
    create_text_field,
    create_date_field,
...
class TestSlackListsClient:
    ...
    async def test_make_request_with_metadata_messages(self):
        ...
        mock_response.json.return_value = {
            ...
            "response_metadata": {"messages": ["[ERROR] missing required field: list_id"]},
        }
        mock_response.raise_for_status.return_value = None
    ...
    @pytest.mark.asyncio
    async def test_update_list_item_success(self):
        ...
        mock_response.json.return_value = {
            "ok": True,
            "item": {"id": "ITEM123", "date_updated": 1707300000},
        }
        mock_response.raise_for_status.return_value = None
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            fields = [{"column_id": "Col123", "rich_text": []}]
            result = await client.update_list_item("F123", "ITEM123", fields)

            assert result["ok"] is True
            assert result["item"]["id"] == "ITEM123"

    @pytest.mark.asyncio
    async def test_update_list_item_error(self):
        """Test item update with API error"""
        client = SlackListsClient("test-token")
        ...
        mock_response.json.return_value = {
            ...
            "response_metadata": {"messages": ["[ERROR] The specified item does not exist"]},
        }
        mock_response.raise_for_status.return_value = None
        ...
        with patch("httpx.AsyncClient") as mock_client:
            ...
            with pytest.raises(SlackListsError) as exc_info:
                await client.update_list_item("F123", "INVALID", [])

            error_message = str(exc_info.value)
            assert "item_not_found" in error_message
            assert "The specified item does not exist" in error_message
